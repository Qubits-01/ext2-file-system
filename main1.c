...
// Function prototypes.
int extractFileObjs(FILE *ext2FS, char *filePath);
...
int main(int argc, char *argv[])
{
    ...
    if (argc == 3)
    {
        // Implicitly begins from the root directory.
        extractFileObjs(ext2FS, argv[2]);
    }
    // ------------------------------------------------------------------------
...
int extractFileObjs(FILE *ext2FS, char *filePath)
{
    ...
    do
    {
        ...
        do
        {
            ...
            // get the inode number and parse the inode.
        ...
        // Reaching this point means that the current token is not found in the
        // current directory. Print an error message and exit the program.
        fprintf(stderr, "INVALID PATH\n");
        exit(-1);
    ...
    // Print the last inode data in the seen nodes list.
    struct Inode *lastInode = (struct Inode *)seenNodesList->prev->data;
    printf("inode number: %d\n", lastInode->type);
    printf("file size: %d\n", lastInode->FSizeLower);
